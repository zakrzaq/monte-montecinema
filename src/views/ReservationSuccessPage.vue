...
const selectedMovie = computed(() => {
  return (
    movieStore.movieById(bookingStore.selectedReservation.movie) || {
      title: "",
    }
  );
});
const reservationEmail = computed(() => {
  return bookingStore.customerEmail
    ? bookingStore.customerEmail
...
<template>
  <div class="success">
    <h1 class="success__title">
      Hell yeah! <br />
      <span class="success__title--red"
        >You booked {{ bookedTickets.length }} tickets</span
      >
    </h1>
    ...
    <div class="success__buttons">
      <BaseButton size="medium" @click="ctaLocation">{{ ctaText }}</BaseButton>
    </div>
  </div>
</template>
